...
bool InSequences::traverseInSequence(Sequence *sequence)
{ // traverse the sequence to split at gaps and measure sequence properties
    ...
    unsigned int
        currId = 0,
        nextId = 0,           // temporarily store the id of a segment to connect gaps
        dist = 0,             // gap size
    ...
    { // this is the first time we see this path name
    ...
    for (char &base : *sequence->sequence)
    {
        ...
        case 'x':
        {
            ...
            if (!wasN && pos > 0)
            { // gap start and gap not at the start of the sequence
...
bool InSequences::traverseInSegment(Sequence *sequence, std::vector<Tag> inSequenceTags)
{ // traverse the segment
    ...
    for (char &base : *sequence->sequence)
    {
        ...
        case '*':
        {
            ...
            auto tag = find_if(inSequenceTags.begin(), inSequenceTags.end(), [](Tag &obj)
                               { return checkTag(obj.label, "LN"); }); // find if length tag is present in the case sequence is missing
...
void InSequences::appendSequence(Sequence *sequence)
{ // method to append a new sequence from a fasta

    threadPool.queueJob([=]
                        { return traverseInSequence(sequence); });
...
void InSequences::appendSegment(Sequence *sequence, std::vector<Tag> inSequenceTags)
{ // method to append a new segment from a gfa

    lg.verbose("Segment read");

    threadPool.queueJob([=]
                        { return traverseInSegment(sequence, inSequenceTags); });
...
InSegment *InSequences::getInSegment(unsigned int sId)
{
    ...
    auto inSegment = find_if(inSegments.begin(), inSegments.end(), [sId](InSegment *obj)
                             { return obj->getuId() == sId; }); // given a uId, find it in nodes
...
InPath InSequences::getInPath(unsigned int pId)
{
    ...
    auto inPath = find_if(inPaths.begin(), inPaths.end(), [pId](InPath &obj)
                          { return obj.getpUId() == pId; }); // given a uId, find it in nodes
...
void InSequences::evalNstars(char type, uint64_t gSize)
{ // switch between scaffold, contig, gap while computing N* statistics
...
void InSequences::evalAuN(char type, uint64_t gSize)
{ // switch between scaffold, contig, gap while computing N* statistics
...
void InSequences::computeAuN(std::vector<uint64_t> &lens, double &auN, double *auNG, uint64_t gSize)
...
std::vector<unsigned int> InSequences::computeContigDistrib()
{
    std::vector<unsigned int> distribReturn;

    for (std::vector<uint64_t>::iterator contigLen = contigLens.begin(); contigLen != contigLens.end(); contigLen++)
    {
        // vector of distribution of contig lengths < 10 < 50 < 100 < 500 < 1000 < 2500 < 5000 < over
        // Contig distribution 0-10, 10-50, 50-100, 100-500, 500-150
        if (*contigLen < 10)
            contigDistrib[0]++;
        else if (*contigLen < 50)
            contigDistrib[1]++;
        else if (*contigLen < 100)
            contigDistrib[2]++;
        else if (*contigLen < 500)
            contigDistrib[3]++;
        else if (*contigLen < 1000)
            contigDistrib[4]++;
        else if (*contigLen < 2500)
            contigDistrib[5]++;
        else if (*contigLen < 5000)
            contigDistrib[6]++;
        else if (*contigLen < 10000)
            contigDistrib[7]++;
        else
            contigDistrib[8]++;
    }

    for (std::vector<uint64_t>::iterator i = contigDistrib.begin(); i != contigDistrib.end(); i++)
    {
        distribReturn.push_back(*i);
    }

    return distribReturn;
}

std::vector<unsigned int> InSequences::computeSegmentDistrib()
{
    std::vector<unsigned int> distribReturn;
    ...
    for (std::vector<InSegment *>::iterator inSegment = inSegments.begin(); inSegment != inSegments.end(); inSegment++)
    {
        ...
        int segmentLen = (*inSegment)->getInSequence().size();
        // vector of distribution of contig lengths < 10 < 50 < 100 < 500 < 1000 < 2500 < 5000 < over
        // Contig distribution 0-10, 10-50, 50-100, 100-500, 500-150
        if (segmentLen < 10)
            segmentDistrib[0]++;
        else if (segmentLen < 50)
            segmentDistrib[1]++;
        else if (segmentLen < 100)
            segmentDistrib[2]++;
        else if (segmentLen < 500)
            segmentDistrib[3]++;
        else if (segmentLen < 1000)
            segmentDistrib[4]++;
        else if (segmentLen < 2500)
            segmentDistrib[5]++;
        else if (segmentLen < 5000)
            segmentDistrib[6]++;
        else if (segmentLen < 10000)
            segmentDistrib[7]++;
        else
            segmentDistrib[8]++;
    }

    for (std::vector<uint64_t>::iterator i = segmentDistrib.begin(); i != segmentDistrib.end(); i++)
    {
        distribReturn.push_back(*i);
    }
...
std::vector<InGap> InSequences::getGaps()
{ // return gfa gaps vector

    return inGaps;
}

std::vector<InEdge> *InSequences::getEdges()
{ // return gfa edge vector
...
void InSequences::sortPathsByOriginal()
{
    ...
    sort(inPaths.begin(), inPaths.end(), [](InPath &one, InPath &two)
         { return one.getSeqPos() < two.getSeqPos(); });
}
...
void InSequences::sortPathsByNameAscending()
{
    ...
    sort(inPaths.begin(), inPaths.end(), [](InPath &one, InPath &two)
         { return one.getHeader() < two.getHeader(); });
}
    ...
    auto comp = [&](InPath &one, InPath &two) -> bool { // lambda function for custom sorting
        ...
        for (std::vector<PathComponent>::iterator component = pathComponents.begin(); component != pathComponents.end(); component++)
        {
            ...
            if (component->type == SEGMENT)
            {
                ...
                auto sId = find_if(inSegments.begin(), inSegments.end(), [uId](InSegment *obj)
                                   { return obj->getuId() == uId; }); // given a node Uid, find it

                if (sId != inSegments.end())
                {
                    sIdx = std::distance(inSegments.begin(), sId);
                } // gives us the segment index
            ...
            else
            {
                ...
                auto gId = find_if(inGaps.begin(), inGaps.end(), [uId](InGap &obj)
                                   { return obj.getuId() == uId; }); // given a node Uid, find it

                if (gId != inGaps.end())
                {
                    gIdx = std::distance(inGaps.begin(), gId);
                } // gives us the segment index
...
void InSequences::buildGraph(std::vector<InGap> const &gaps)
{ // graph constructor
...
void InSequences::buildEdgeGraph(std::vector<InEdge> const &edges)
{ // graph constructor
...
void InSequences::dfsEdges(unsigned int v, unsigned int *componentLength)
{ // Depth First Search to explore graph connectivity
    ...
    auto sId = find_if(inSegments.begin(), inSegments.end(), [v](InSegment *obj)
                       { return obj->getuId() == v; }); // given a node Uid, find it
    ...
    if (adjEdgeList.at(v).size() > 1)
    { // if the vertex has more than one edge
    ...
    else if (adjEdgeList.at(v).size() == 1)
    { // this is a single dead end
    ...
    else if (adjEdgeList.at(v).size() == 0)
    { // disconnected component (double dead end)
    ...
    for (auto i : adjEdgeList[v])
    { // recur for all forward vertices adjacent to this vertex
...
void InSequences::dfsScaffolds(unsigned int v, unsigned int *scaffSize, unsigned int *A, unsigned int *C, unsigned int *G, unsigned int *T, unsigned int *lowerCount) // Depth First Search to explore graph connectivity
{
    ...
    auto it = find_if(inSegments.begin(), inSegments.end(), [&v](InSegment *obj)
                      { return obj->getuId() == v; }); // given a vertex id, search it in the segment vector

    if (it != inSegments.end())
    {
        idx = std::distance(inSegments.begin(), it);
    } // if found, get its index

    if (adjListFW.at(v).size() == 1 && adjListBW.at(v).size() == 1 && !(adjListFW.at(v).at(0).segmentId == adjListBW.at(v).at(0).segmentId) && !backward)
    { // if the vertex has exactly one forward and one backward connection and they do not connect to the same vertex (internal node)
    ...
    else if (adjListFW.at(v).size() == 0 && adjListBW.at(v).size() == 1)
    { // this is the final vertex without gaps
    ...
    else if (adjListFW.at(v).size() == 1 && adjListBW.at(v).size() == 2)
    { // this is the final vertex with terminal gap

        lg.verbose("node: " + idsToHeaders[v] + " --> case c: end node, forward direction, final gap");

        if (adjListBW.at(v).at(0).segmentId != v)
        { // make sure you are not using the terminal edge to ascertain direction in case it was edited by sak
    ...
    else if (adjListFW.at(v).size() == 1 && adjListBW.at(v).size() == 1 && !(adjListFW.at(v).at(0).segmentId == adjListBW.at(v).at(0).segmentId) && backward)
    { // this is an intermediate vertex, only walking back
    ...
    else if (adjListFW.at(v).size() == 0 && adjListBW.at(v).size() == 0)
    { // disconnected component

        lg.verbose("node: " + idsToHeaders[v] + " --> case e: disconnected component");
    }
    else if (adjListFW.at(v).size() == 1 && adjListBW.at(v).size() == 0)
    { // this is the first vertex without gaps
    ...
    else if (adjListFW.at(v).size() == 2 && adjListBW.at(v).size() == 1)
    { // this is the first vertex with a terminal gap
    ...
    else if (adjListFW.at(v).size() == 1 && adjListBW.at(v).size() == 1 && adjListFW.at(v).at(0).segmentId == adjListBW.at(v).at(0).segmentId)
    { // if the vertex has exactly one forward and one backward connection and they connect to the same vertex (disconnected component with gap)
    ...
    *lowerCount += inSegments[idx]->getLowerCount();
    ...
    for (auto i : adjListBW[v])
    { // recur for all backward vertices adjacent to this vertex
...
{ // if two contigs are provided, remove all edges connecting them, if only one contig is provided remove all edges where it appears
    ...
    while (pathIt != end(inPaths))
    {
        ...
        for (std::vector<PathComponent>::iterator componentIt = pathComponents.begin(); componentIt != pathComponents.end();)
        {
            ...
            if (componentIt->type == GAP)
            {
                ...
                auto gId = find_if(inGaps.begin(), inGaps.end(), [uId](InGap &obj)
                                   { return obj.getuId() == uId; }); // given a gap Uid, find it
    ...
    { // update statistics to reflect the removal
...
std::vector<InGap> InSequences::getGap(std::string *contig1, std::string *contig2)
{ // if two contigs are provided, returns all gaps connecting them, if only one contig is provided returns all gaps where it appears
    ...
    if (contig2 != NULL)
    {
        ...
        auto gId = find_if(inGaps.begin(), inGaps.end(), [sUId1, sUId2](InGap &obj)
                           { return ( // given two vertex ids, search the gap that connects them
...
std::vector<unsigned int> InSequences::removeGaps(std::string *contig1, std::string *contig2)
{ // if two contigs are provided, remove all gaps connecting them, if only one contig is provided remove all gaps where it appears
    ...
    else
    {
        ...
        while (it != end(inGaps))
        {
            ...
            auto gId = find_if(it, inGaps.end(), [sUId1](InGap &obj)
                               { return obj.getsId1() == sUId1 || obj.getsId2() == sUId1; }); // check whether an edge containing the node was found
...
bool InSequences::deleteSegment(std::string *contig1)
{
    ...
    if (contig1 != NULL)
    {
        ...
        auto sId = find_if(inSegments.begin(), inSegments.end(), [sUId](InSegment *obj)
                           { return obj->getuId() == sUId; }); // given a node uId, find it
...
void InSequences::removePath(unsigned int pUId, bool all, bool silent)
{
    ...
    auto pathIt = find_if(inPaths.begin(), inPaths.end(), [pUId](InPath &obj)
                          { return obj.getpUId() == pUId; }); // given a path pUId, find it
    ...
    if (all)
    {
        ...
        for (auto &component : pathComponents)
        {
            ...
            if (component.type == SEGMENT)
            {
                ...
                auto sId = find_if(inSegments.begin(), inSegments.end(), [cUId](InSegment *obj)
                                   { return obj->getuId() == cUId; }); // given a node Uid, find it
            ...
            else if (component.type == GAP)
            {
                ...
                auto gId = find_if(inGaps.begin(), inGaps.end(), [cUId](InGap &obj)
                                   { return obj.getuId() == cUId; }); // given a node Uid, find it
...
void InSequences::removeGap(unsigned int gUId, bool silent)
{
    ...
    auto gapIt = find_if(inGaps.begin(), inGaps.end(), [gUId](InGap &obj)
                         { return obj.getuId() == gUId; }); // given a path pUId, find it
...
void InSequences::resizeGap(std::string gHeader, unsigned int size)
{
    ...
    auto gapIt = find_if(inGaps.begin(), inGaps.end(), [gHeader](InGap &obj)
                         { return obj.getgHeader() == gHeader; }); // given a path pUId, find it
...
void InSequences::removePathsFromSegment(unsigned int uId)
{
    ...
    for (InPath &inPath : inPaths)
    {
        ...
        auto pathIt = find_if(pathComponents.begin(), pathComponents.end(), [uId](PathComponent &obj)
                              { return obj.id == uId; }); // given a node uId, find if present in the given path
...
void InSequences::removeSegmentInPath(unsigned int suId, InGap gap)
{
    ...
    for (InPath &inPath : inPaths)
    {
        ...
        auto pathIt = find_if(pathComponents.begin(), pathComponents.end(), [suId](PathComponent &obj)
                              { return obj.id == suId; }); // given a node uId, find if present in the given path
...
void InSequences::joinPaths(std::string pHeader, unsigned int pUId1, unsigned int pUId2, std::string gHeader, unsigned int gUId, char pId1Or, char pId2Or, unsigned int dist, unsigned int start1, unsigned int end1, unsigned int start2, unsigned int end2)
{
    ...
    if (got == headersToIds.end())
    { // this is the first time we see this path
    ...
    auto pathIt = find_if(inPaths.begin(), inPaths.end(), [pUId1](InPath &obj)
                          { return obj.getpUId() == pUId1; }); // given a path pUId, find it
    ...
    pathIt = find_if(inPaths.begin(), inPaths.end(), [pUId2](InPath &obj)
                     { return obj.getpUId() == pUId2; }); // given a path pUId, find it
...
InPath InSequences::joinPathsByComponent(std::string seqHeader, unsigned int uId1, unsigned int uId2, unsigned int uId3)
{
    ...
    { // add first path
        ...
        auto pathIt = find_if(pathComponents.begin(), pathComponents.end(), [uId1](PathComponent &obj)
                              { return obj.id == uId1; }); // given a node uId, find if present in the given path
    ...
    { // add second path
        ...
        auto pathIt = find_if(pathComponents.begin(), pathComponents.end(), [uId3](PathComponent &obj)
                              { return obj.id == uId3; }); // given a node uId, find if present in the given path
...
void InSequences::splitPath(unsigned int guId, std::string pHeader1, std::string pHeader2)
{
    ...
    { // search through all paths
        ...
        auto pathIt = find_if(pathComponents.begin(), pathComponents.end(), [guId](PathComponent &obj)
                              { return obj.id == guId; }); // given a node uId, find if present in the given path
...
void InSequences::trimPath(std::vector<PathComponent> *pathComponents, unsigned int start, unsigned int end)
{
    ...
    for (std::vector<PathComponent>::iterator component = pathComponents->begin(); component != pathComponents->end(); component++)
    {
        ...
        { // account for editing of the start coordinate but not the end coordinate

            component->end = compOriginalSize;
        }
        else if (component->end > 0 && component->start == 0)
        { // account for editing of the end coordinate but not the start coordinate

            component->start = 1;
        }
        else if (component->start == 1 && component->end == compOriginalSize)
        { // if the result of trimming restores the original size of the component, no need to adjust the internal coordinates
    ...
    {
        fprintf(stderr, "Error: Path size after trimming (%u) differs from expected size after trimming (%u). Terminating.\n", actualSize, end - start + 1);
        exit(1);
    }
}
...
void InSequences::trimComponent(PathComponent &component, int start, int end)
{
    ...
    if (component.orientation == '+' || component.orientation == '0')
    { // we only change the end coordinate if the component wasn't already flipped, otherwise we edit the start
...
int InSequences::getComponentSize(PathComponent &component, bool original)
{
    ...
    if (component.type == SEGMENT)
    {
        ...
        auto inSegment = find_if(inSegments.begin(), inSegments.end(), [cUId](InSegment *obj)
                                 { return obj->getuId() == cUId; }); // given a node Uid, find it
    ...
    else
    {
        ...
        auto inGap = find_if(inGaps.begin(), inGaps.end(), [cUId](InGap &obj)
                             { return obj.getuId() == cUId; }); // given a node Uid, find it
...
void InSequences::findBubbles()
{
    ...
    for (InSegment *segment : inSegments)
    {
        ...
        if (!visited[sUId] && !deleted[sUId])
        { // check if the node was already visited

            lg.verbose("The node was not yet visited or deleted");

            if (adjEdgeList.at(sUId).size() == 2 && adjEdgeList.at(sUId).at(0).orientation0 != adjEdgeList.at(sUId).at(1).orientation0)
            { // if it has exactly two edges with different orientation it could be a het region of the bubble
                ...
                if (adjEdgeList.at(sUId1).size() >= 2 && adjEdgeList.at(sUId2).size() >= 2)
                { // both nodes need at least two edges to be a bubble
                    ...
                    for (auto edge1 : adjEdgeList.at(sUId1))
                    {
                        ...
                        if (edge1.orientation1 == sId1Or && edge1.id != sUId)
                        { // we are checking edges on the side of the potential bubble for node1, avoiding the original node

                            lg.verbose("Evaluating node: " + idsToHeaders[edge1.id] + " (uId: " + std::to_string(edge1.id) + ")");

                            if (edge1.id == sUId2)
                            { // this is a potential insertion
                            ...
                            else
                            {
                                ...
                                for (auto edge2 : adjEdgeList.at(sUId2))
                                {
                                    ...
                                    if (edge2.orientation1 == sId2Or && edge1.id == edge2.id)
                                    { // we are checking edges on the side of the potential bubble for node2 and that it connects to the same node as node1
