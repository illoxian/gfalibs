...
bool Report::seqReport(InSequences &inSequences, int &outSequence_flag)
{                                      // method to output the summary statistics for each sequence
...
bool Report::outFile(InSequences &inSequences, std::string file, UserInput &userInput, int splitLength)
{                                      // method to output new sequence opposed to sequence report
    std::cout << std::fixed;           // disables scientific notation
    std::cout << std::setprecision(2); // 2 decimal poinst
    ...
    if (gzip && outFile)
    { // if the requested output is gzip compressed and should be outputted to a file
    ...
    { // else the output is not written to a file
        ...
        if (gzip)
        { // if the output to stdout needs to be compressed we use the appropriate stream
        ...
        { // else we use a regular cout stream
    ...
    { // this switch allows us to generate the output according to the input request and the unordered map. If the requested output format is not in the map we fall back to the undefined case (0)
            ...
            *stream << std::endl;
            ...
            for (std::vector<PathComponent>::iterator component = pathComponents.begin(); component != pathComponents.end(); component++)
            {
                ...
                if (component->type == SEGMENT)
                {
                    ...
                    {
                        std::cout << "Error: cannot find path component" << std::endl;
                        exit(1);
                ...
                { // this is just a prototype, need to handle cigar

                    auto edge = find_if(inEdges->begin(), inEdges->end(), [uId](InEdge &obj)
                                        { return obj.geteUId() == uId; }); // given a node Uid, find it
                ...
                else
                {
                    ...
                    auto gap = find_if(inGaps->begin(), inGaps->end(), [uId](InGap &obj)
                                       { return obj.getuId() == uId; }); // given a node Uid, find it
    ...
    if (gzip && outFile)
    { // if we wrote to file as gzip, we add the footer and close

        zfout.close();
    }
    else if (gzip && !outFile)
    { // if we streamed as gzip, we add the footer and close

        zout.close();
    }

    if (outFile)
    { // if we wrote to file, we close it
...
bool Report::outCoord(InSequences &inSequences, char bedOutType, bool sizeOnly)
{                                      // method to output the coordinates of each feature
    std::cout << std::fixed;           // disables scientific notation
    std::cout << std::setprecision(2); // 2 decimal poinst
    ...
    for (InPath inPath : inSequences.getInPaths())
    {
        ...
        // default includes 'a'
        default:
        { // both contigs and gaps in .agp format
...
bool Report::reportStats(InSequences &inSequences, uint64_t gSize)
{                                      // method to output all summary statistics for the entire sequence set
    std::cout << std::fixed;           // disables scientific notation
    std::cout << std::setprecision(2); // 2 decimal poinst
    ...
    std::cout << output("Contig distribution 0-10, 10-50, 50-100, 100-500, 500-1000, 1000-2500, 2500-5000, 5000-10000, over 10000") << "\n";
    std::cout << output("Contig distribution");
    for (auto &i : inSequences.computeContigDistrib())
    {
        std::cout << i << " ";
    }
    std::cout << "\n";
    ...
    std::cout << output("Segment distribution") << "\n";
    for (auto &i : inSequences.computeSegmentDistrib())
    {
        std::cout << i << " ";
    }
    std::cout << "\n";
    ...
    if (edgeN > 0)
    {
        ...
        { // loop through all nodes
            ...
            if (!inSequences.getVisited(inSegment->getuId()) && !inSequences.getDeleted(inSegment->getuId()))
        ...
        if (outBubbles_flag)
        {
            ...
            for (Bubble bubble : *inSequences.getBubbles())
            { // loop through all nodes
...
bool Report::nstarReport(InSequences &inSequences, uint64_t gSize)
{                                      // method to generate all N** reports
    std::cout << std::fixed;           // disables scientific notation
    std::cout << std::setprecision(2); // 2 decimal poinst
