...
class InSequences
{ // collection of InSegment and inGap objects and their summary statistics
...
private:
    ...
    std::vector<uint64_t> contigDistrib{0, 0, 0, 0, 0, 0, 0, 0, 0};
    std::vector<uint64_t> segmentDistrib{0, 0, 0, 0, 0, 0, 0, 0, 0};
...
public:
    ...
    std::vector<unsigned int> computeContigDistrib();

    std::vector<unsigned int> computeSegmentDistrib();
